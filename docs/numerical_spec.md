...
|---|---|
| Version | 1.0 |
| Status | Frozen at Day 0, before any FE model exists |
...
| L/t | **10.0** | Slenderness — kept ≥ 8 so beam theory remains valid |
...
| **Timoshenko, P_T** | **3.6455 N** ← *the FE target* |
| Acceptance band (±2 %) | 3.573 – 3.718 N |
| Root stress (linear) | 110.25 MPa |
| Root strain | 1.1250 % |
...
**FE with a perfect root clamp should reproduce P_T, not P_EB.** The direction and
magnitude of the expected deviation are predicted in advance: approximately 0.8 % below
Euler–Bernoulli, because of transverse shear.
...
housing. Prescribed tip displacement y = 0.60 mm, smooth-step over 5 ms (≈ 26 × T₁),
v ≈ 120 mm/s, `DT2MS = 0`.

- Analytical reference: `scripts/analytical.py`
- Abaqus/Standard: `C3D8I` baseline, `C3D8R` comparison, `C3D8` run deliberately to
  demonstrate shear locking
- LS-DYNA: `ELFORM -1` baseline, `ELFORM 1` with `IHQ=6, QM=0.1` comparison
- Mesh convergence: 1 / 2 / 4 / 8 elements through thickness (h = 0.8 / 0.4 / 0.2 / 0.1 mm)
- Equilibrium check: `SPCFORC` at the root must equal `BNDOUT` at the driven tip
...
at one converged mesh. Eight runs, not twenty.
...
| Element formulation | Stage 1 | C3D8I ≈ ELFORM −1; C3D8 locks |
