...
in `analytical.py` before anything was run. The tooth stiffens the lance by 1.1 %. The load
...
output under fixed names into whatever directory it is started in. `ls-dyna/stage3/RUNNING.md`
has the commands.

Geometry is generated by `scripts/make_stage3_models.py` — the tooth breakpoints land
...
retention face, against a value written into `analytical.py` before any deck existed:
